...
- **Quality eval** (`npm run eval:search`) runs 32 server-surface queries plus 16 tool-surface queries and scores precision@1/3, weighted, intent accuracy, tool-evidence rate and false-tool rate against the synthetic fixture. Baselines are stored **per provider/model** (`tests/fixtures/search-eval-baseline.json`), so switching models never false-fails the gate — a model without its own baseline bootstraps it. **The gate is dynamic:** each model's recent scores (last 10 runs, stored as `__history` in the baseline) set its floor — `mean − jitter` for the pass band, twice that for the fail band. Scores inside the band **pass with a warning** (embedding models drift a point or two run-to-run; one bad day is not a regression), scores below **fail**. The canonical baseline ratchets up instantly, follows down only after several consecutive low runs, and failed runs are never recorded. Tunables: `--tolerance` (minimum wiggle room, default 2 pts ≈ one rank flip), `--history-limit`, `--jitter-multiplier`, `--warn-bands`. Deliberate re-floor (e.g. after a model update): `npm run eval:search:update -- --force-update-baseline` — accepts the current scores as the new normal and resets the history; it works even on a red run. The gate asserts the server weighted/p@3, the tool p@3 and the intent accuracy.
