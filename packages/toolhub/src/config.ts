...
/**
 * Parse a single `.env` line into a key/val pair.
 * Handles: optional `export ` prefix, single/double-quoted values (with
 * trailing comments allowed after the closing quote), and trailing
 * ` # comment` on unquoted values. Returns null for blank/comment/invalid
 * lines. Unquoted `#` inside a value (e.g. URL fragments) is preserved —
 * only a `#` preceded by whitespace starts a comment.
 */
export function parseEnvLine(line: string): { key: string; val: string } | null {
  ...
  if (quote === '"' || quote === "'") {
    ...
    // Unterminated quote — treat the rest as the value without the opener.
    return { key, val: out };
  }
